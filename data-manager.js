...
const FIRESTORE_COLLECTION = 'clt_research_data';
const SETTINGS_COLLECTION = 'experiment_settings';
...
async function saveToFirestoreREST(data) {
  ...
  try {
    const response = await fetch(
      `https://firestore.googleapis.com/v1/projects/${FIREBASE_CONFIG.projectId}/databases/(default)/documents/${FIRESTORE_COLLECTION}?key=${FIREBASE_CONFIG.apiKey}`,
      {
        method: 'POST',
    ...
    console.log('✅ Data saved via Firestore REST:', result.name || data.sessionId);
    return true;
  } catch (error) {
...
async function loadFromFirestoreREST() {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 8000);
  try {
    const response = await fetch(
      `https://firestore.googleapis.com/v1/projects/${FIREBASE_CONFIG.projectId}/databases/(default)/documents/${FIRESTORE_COLLECTION}?orderBy=clientTimestamp%20desc&key=${FIREBASE_CONFIG.apiKey}`,
      { signal: controller.signal }
    );

    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`REST load failed: ${response.status} ${errorText}`);
    }
    ...
    console.error('❌ Firestore REST load error:', error);
    return [];
  } finally {
    clearTimeout(timeoutId);
  }
...
async function saveToFirestore(data) {
  if (!useFirebase || !db) {
    return await saveToFirestoreREST(data);
  }
  try {
...
async function loadFromFirestore() {
  if (!useFirebase || !db) return await loadFromFirestoreREST();
  try {
    ...
    return await loadFromFirestoreREST();
  }
}
...
async function saveSettingsToFirestore(settings) {
  ...
  try {
    await db.collection(SETTINGS_COLLECTION).doc('main').set({
      ...settings,
...
function listenToSettingsChanges(callback) {
  ...
  return db.collection(SETTINGS_COLLECTION).doc('main')
    .onSnapshot(doc => {
      if (doc.exists) {
        const settings = doc.data();
        delete settings.lastUpdated;
