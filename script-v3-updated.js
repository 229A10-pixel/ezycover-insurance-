...
function showSurveyModal() {
  ...
  let html = '';
  html += `<div class="survey-intro">${scaleGuide}</div>`;

  let currentSection = null;
  questions.forEach((question) => {
    if (question.type === 'intro') {
      html += `<div class="survey-intro survey-intro-brief">${question.text}</div>`;
      return;
    }
    ...
    if (question.type === 'section') {
      ...
      html += `<div class="survey-section-title">${question.title}</div>`;
      if (question.note) {
        html += `<div class="survey-section-intro">${question.note}</div>`;
      }
      currentSection = question.title;
      return;
    }

    html += `<div class="survey-question-card">`;
    ...
    if (question.type === 'single_choice') {
      ...
      for (let i = 1; i <= question.scale; i++) {
        html += `
          <button type="button" class="survey-scale-btn" onclick="selectSurveyAnswer('${question.id}', ${i}, this, '${question.variable}')">
            ${i}
          </button>
        `;
      }
      html += `</div>`;
      html += `<div class="survey-scale-scale-labels">`;
      html += `<span>1 = 十分同意 / Strongly agree</span>`;
      html += `<span>5 = 十分不同意 / Strongly disagree</span>`;
      html += `</div>`;
    }
