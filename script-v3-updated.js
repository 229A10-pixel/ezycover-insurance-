...
function setupEventListeners() {
  ...
  document.getElementById('custom-response-time')?.addEventListener('change', async function() {
    ...
    if (typeof dataManager !== 'undefined' && dataManager.saveSettings) {
      try {
        synced = await dataManager.saveSettings({ responseTime: responseTime });
        console.log('✅ Response time sync result:', synced);
      } catch (err) {
        console.error('❌ Sync error:', err);
      }
    }
    
    alert(synced ? ('修改成功！响应时间: ' + responseTime + 'ms') : '本地已修改，但云端同步失败，请稍后重试');
...
async function handleResponseTimeChange(event) {
  ...
  alert(synced ? ('修改成功！响应时间: ' + time + 'ms') : '本地已修改，但云端同步失败，请稍后重试');
...
function showInsuranceModal() {
  ...
  let html = `
    <div class="insurance-option" onclick="selectInsurance(this, 'basic', 39)">
      <div class="insurance-option-icon">🧳</div>
      <div class="insurance-option-name">${lang.basicInsurance}</div>
      <div class="insurance-option-price">¥39 / 天</div>
      <div class="insurance-option-coverage">${lang.basicCoverage}</div>
    </div>
    <div class="insurance-option" onclick="selectInsurance(this, 'premium', 69)">
      <div class="insurance-option-icon">🛡️</div>
      <div class="insurance-option-name">${lang.premiumInsurance}</div>
      <div class="insurance-option-price">¥69 / 天</div>
      <div class="insurance-option-coverage">${lang.premiumCoverage}</div>
    </div>
    <div class="insurance-option" onclick="selectInsurance(this, 'luxury', 99)">
      <div class="insurance-option-icon">✨</div>
      <div class="insurance-option-name">${lang.luxuryInsurance}</div>
      <div class="insurance-option-price">¥99 / 天</div>
      <div class="insurance-option-coverage">${lang.luxuryCoverage}</div>
    </div>
...
function buildSurveyHTML(questions, scaleGuide) {
  let html = `<div class="survey-intro">${escapeHtml(scaleGuide)}</div>`;
  let sectionOpen = false;

  questions.forEach((question) => {
    if (question.type === 'intro') {
      html += `<div class="survey-intro survey-intro-brief">${escapeHtml(question.text)}</div>`;
      return;
    }
    ...
    if (question.type === 'section') {
      ...
      html += `<div class="survey-section-title">${escapeHtml(question.title)}</div>`;
      if (question.note) {
        html += `<div class="survey-section-intro">${escapeHtml(question.note)}</div>`;
      }
      sectionOpen = true;
      return;
    }

    html += renderSurveyQuestion(question);
  });
...
function renderSurveyQuestion(question) {
  ...
  if (question.type === 'single_choice') {
    ...
    return `
      <div class="survey-question-card">
        <div class="survey-question-text">${escapeHtml(labelText)}</div>
        <div class="survey-multiple-choice">
          ${optionList.map(option => `
            <label class="survey-checkbox">
              <input type="radio" name="${question.id}" value="${escapeHtmlAttr(option)}" onchange="saveSurveyAnswer('${question.id}', this.value, '${question.variable}')" />
  ...
  const buttons = Array.from({ length: 5 }, (_, index) => {
    const value = index + 1;
    return `<button type="button" class="survey-scale-btn" onclick="selectSurveyAnswer('${question.id}', ${value}, this, '${question.variable}')">${value}</button>`;
  }).join('');

  return `
    <div class="survey-question-card">
      <div class="survey-question-text">${escapeHtml(labelText)}</div>
      <div class="survey-scale">${buttons}</div>
      <div class="survey-scale-scale-labels">
        <span>1 = 十分同意 / Strongly agree</span>
        <span>5 = 十分不同意 / Strongly disagree</span>
      </div>
    </div>
  `;
}
...
async function submitSurvey() {
  ...
  // 获取所有实际问题（排除说明等）
  const actualQuestions = questions.filter(q => q.type === 'likert' || q.type === 'single_choice');
  ...
  try {
    ...
    alert(appState.currentLanguage === 'zh' ? '提交失败，请稍后重试。' : 'Submission failed. Please try again.');
    if (submitButton) {
    ...
    <div class="content-container">
        ...
        <div class="module">
          ...
          <form class="flight-form">
            ...
            <div class="form-group">
              ...
              <div class="form-hint">${appState.currentLanguage === 'zh' ? '始发地固定为香港' : 'Departure is fixed as Hong Kong'}</div>
            </div>
      ...
      <div class="insurance-module">
        <div class="module">
          ...
          <!-- 确认选择按钮 -->
            <button type="button" class="insurance-confirm-btn" onclick="confirmInsuranceSelection(event)">${lang.confirmButton}</button>
        </div>
      </div>
    </div>
...
function generateDepartureOptions() {
  const city = DEPARTURE_CITY[appState.currentLanguage];
  return `<option value="${city.code}" selected>${city.name} (${city.country})</option>`;
}
...
// 💰 确认主页面保险选择
// ============================================
function confirmInsuranceSelection(event) {
  ...
  try {
    ...
    console.error('❌ Failed to open survey modal:', error);
    alert(appState.currentLanguage === 'zh' ? '问卷打开失败，请刷新后重试。' : 'Failed to open the survey. Please refresh and try again.');
  }
}
